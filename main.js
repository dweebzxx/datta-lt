...
class Visualizer {
    ...
    render(data, config) {
        ...
        // Inject Custom Titles if they exist in UI
...
class FilterEngine {
    ...
    generateFilters() {
        ...
        btn.onclick = () => {
             ...
             select.className = 'w-full border p-1 rounded text-xs mb-2 dark:bg-gray-800 dark:border-gray-600';
             select.innerHTML = '<option value="">Select Column...</option>';
             this.dm.headers.forEach(h => select.add(new Option(h, h)));
             select.onchange = () => {
                 this.addFilterWidget(select.value);
                 select.remove();
             };
             container.insertBefore(select, btn);
        };
    }
    addFilterWidget(column) {
        const container = document.getElementById('filters-container');
        const btn = document.getElementById('btn-add-filter');
        
        if (document.getElementById(`filter-widget-${column}`)) return;
        const uniqueValues = new Set();
        this.dm.processedData.forEach(row => {
            if (row[column] !== undefined) uniqueValues.add(String(row[column]));
        });
        const sortedValues = Array.from(uniqueValues).sort();
        const widget = document.createElement('div');
        widget.id = `filter-widget-${column}`;
        widget.className = 'bg-white/50 dark:bg-gray-800/50 p-2 rounded mb-2 border border-gray-200 dark:border-gray-700';
        const header = document.createElement('div');
        header.className = 'flex justify-between items-center mb-1 font-semibold text-xs uppercase';
        const titleSpan = document.createElement('span');
        titleSpan.textContent = column;
        header.appendChild(titleSpan);
        const closeBtn = document.createElement('button');
        closeBtn.innerHTML = '&times;'; // Safe as it's static
        closeBtn.className = 'text-gray-500 hover:text-red-500';
        closeBtn.onclick = () => {
            widget.remove();
        ...
        list.className = 'max-h-24 overflow-y-auto space-y-1';
        sortedValues.forEach(val => {
            ...
            cb.checked = true; 
            cb.value = val;
            cb.className = 'rounded text-blue-600 focus:ring-blue-500 w-3 h-3';
            cb.addEventListener('change', () => {
                this.updateActiveFilters(column, widget);
            });
            ...
            if (this.dm.codebook.mapping[column] && this.dm.codebook.mapping[column][val]) {
                displayVal = this.dm.codebook.mapping[column][val];
            }
    ...
    updateActiveFilters(column, widget) {
...
class CodebookManager {
    ...
    init() {
        ...
        document.getElementById('map-column-select').addEventListener('change', (e) => {
            this.renderMapTable(e.target.value);
        });
        document.getElementById('save-map').addEventListener('click', () => {
            this.saveMapping();
            document.getElementById('map-modal-overlay').classList.add('hidden');
        });
        document.getElementById('btn-download-codebook').addEventListener('click', () => this.downloadTemplate());
    }
    renderMapTable(column) {
        const tbody = document.getElementById('map-table-body');
        tbody.innerHTML = '';
        if (!column) return;
        const uniqueValues = new Set();
        this.dm.rawData.forEach(row => {
            if (row[column] !== undefined) uniqueValues.add(row[column]);
        });
        const currentMap = this.dm.codebook.mapping[column] || {};
        Array.from(uniqueValues).sort().forEach(val => {
            const tr = document.createElement('tr');
            tr.className = 'bg-white dark:bg-gray-800 border-b hover:bg-gray-50 dark:hover:bg-gray-700';
            const tdOriginal = document.createElement('td');
            tdOriginal.className = 'px-6 py-2';
            tdOriginal.textContent = val;
            const tdMap = document.createElement('td');
            tdMap.className = 'px-6 py-2';
            const input = document.createElement('input');
            input.type = 'text';
            input.className = 'w-full border rounded px-2 py-1 text-sm map-input bg-transparent dark:border-gray-600';
            input.dataset.original = val;
            input.value = currentMap[val] || '';
            input.placeholder = val; 
            tdMap.appendChild(input);
    ...
    saveMapping() {
        const column = document.getElementById('map-column-select').value;
        if (!column) return;
        ...
        inputs.forEach(input => {
            if (input.value && input.value !== input.dataset.original) {
                mapping[input.dataset.original] = input.value;
                hasMapping = true;
            }
        });
        if (hasMapping) {
    ...
    downloadTemplate() {
        ...
        this.dm.headers.forEach(h => {
             const uniqueValues = new Set();
             this.dm.rawData.forEach(row => {
                if (row[h] !== undefined) uniqueValues.add(row[h]);
             });
             uniqueValues.forEach(v => rows.push([h, v, "", ""]));
        });
        ...
        link.setAttribute("download", "codebook_template.csv");
